...
# ==========================================
# 📊 TERMINAL DO INVESTIDOR // LEITURA DIRETA (LOCAL)
# ==========================================
...
# 1. LEITURA DIRETA DE ARQUIVOS LOCAIS
ARQUIVO_MESTRE = "estado_mestre.json"
ARQUIVO_KARV = "estado_karv.json"

@st.cache_resource(ttl=5) # Cache curto (5s) para atualização quase em tempo real
def carregar_dados_locais():
    dados = {"mestre": {}, "karv": {}, "ultima_att": "Aguardando..."}
    
    # Tenta ler o arquivo do Mestre
    if os.path.exists(ARQUIVO_MESTRE):
        try:
            with open(ARQUIVO_MESTRE, 'r') as f:
                dados["mestre"] = json.load(f)
        except Exception as e:
            st.warning(f"Erro ao ler o Mestre: {e}")
            
    # Tenta ler o arquivo do Karv/K-Node
    if os.path.exists(ARQUIVO_KARV):
        try:
            with open(ARQUIVO_KARV, 'r') as f:
                dados["karv"] = json.load(f)
        except Exception as e:
            st.warning(f"Erro ao ler o Karv: {e}")
...
def calcular_indice_confianca(moeda, sinal_mestre, dados_karv):
    ...
    veredicto = "ALTA PROBABILIDADE" if confianca >= 80 else "MÉ বিধ PROBABILIDADE" if confianca >= 60 else "OBSERVAÇÃO"
...
# RENDERIZAÇÃO
ecossistema = carregar_dados_locais()
...
with col1:
    st.markdown("""
        <div class="header-box" translate="no">
            ...
            <div class="subtitle-text">MONITORAMENTO DE DADOS ALGORÍTMICOS B2C (CONEXÃO DIRETA)</div>
        </div>
    """, unsafe_allow_html=True)
with col2:
...
# Extrai os sinais dos dados carregados
