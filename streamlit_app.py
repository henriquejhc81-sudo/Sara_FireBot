...
# Configuração da Página - Visual Clássico e Limpo
...
if 'saldo_usdt' not in st.session_state:
    ...
    st.session_state.queda_autonoma = 0.10
    st.session_state.lucro_autonomo = 0.25

# Banco de Dados Supabase (Otimizado em segundo plano)
...
# Inteligência de Volatilidade da Janela Temporal (Função Interna Ativa)
...
preco_atual = res_preco if res_preco is not None else (st.session_state.historico_precos[-1]['preco'] + random.uniform(-5, 5) if st.session_state.historico_precos else 64199.0)

st.session_state.historico_precos.append({'hora': datetime.now().strftime('%H:%M:%S'), 'preco': preco_atual})
if len(st.session_state.historico_precos) > 25: st.session_state.historico_precos.pop(0)

# Cores e Estilos CSS Minimalistas (Dourado Imperial e Verde Suave)
cor_b = "#10b981" if st.session_state.bot_ativo else "#ef4444"
cor_h = "#059669" if st.session_state.bot_ativo else "#dc2626"

st.markdown(f"""
    <style>
    /* Fundo industrial e fontes limpas */
    .stApp {{ background-color: #0d1117; color: #c9d1d9; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }}
    
    /* Título apenas SARA_FIREBOLT em Dourado imperial elegante */
    h1 {{ color: #d4af37 !important; text-align: left; font-size: 2.2rem; font-weight: 700; letter-spacing: -0.5px; margin-bottom: 25px; }}
    h3 {{ color: #e1b12c !important; font-size: 1.2rem; font-weight: 600; margin-top: 20px; }}
    
    /* Botão Retangular Clássico */
    ...
    /* Cards de Métricas Sem Poluição */
    .metric-container {{ display: flex; gap: 15px; margin-bottom: 15px; }}
    .metric-card {{ background-color: #161b22; border: 1px solid #30363d; padding: 16px 20px; border-radius: 6px; flex: 1; text-align: left; }}
    .metric-title {{ color: #8b949e; font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }}
    .metric-value {{ color: #f0f6fc; font-size: 1.5rem; font-weight: 600; }}
    
    /* Banner de IA em Tom de Fogo Clássico */
    .ia-banner {{ background-color: #1c1912; border-left: 3px solid #d4af37; padding: 12px 16px; border-radius: 4px; color: #e1b12c; font-size: 0.85rem; font-weight: 500; margin-bottom: 15px; }}
    
    /* Boxes de Log Discretos e Elegantes */
    .log-box {{ background-color: #161b22; border: 1px solid #30363d; padding: 12px 16px; border-radius: 6px; color: #8b949e; font-size: 0.85rem; margin-bottom: 8px; font-family: monospace; }}
    .log-box-buy {{ border-left: 3px solid #10b981; color: #f0f6fc; }}
    .log-box-sell {{ border-left: 3px solid #d4af37; color: #f0f6fc; }}
    </style>
""", unsafe_allow_html=True)

# Topo do Painel Simplificado
st.title("SARA_FIREBOLT")

# Controle do Radar Clássico
...
# Banner de Monitoramento Adaptativo
if st.session_state.bot_ativo:
...
# Minigráfico Técnico Fluido e Integrado ao Fundo
df_p = pd.DataFrame(st.session_state.historico_precos)
fig = go.Figure(go.Scatter(x=df_p['hora'], y=df_p['preco'], mode='lines', line=dict(color='#d4af37', width=1.5), fill='tozeroy', fillcolor='rgba(212, 175, 55, 0.005)'))
fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=0, b=0), height=80, xaxis=dict(showgrid=False, visible=False), yaxis=dict(showgrid=False, visible=False))
st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

# Lógica Interna de Execução Quantitativa
if st.session_state.bot_ativo:
